...
def adjacencyList(edges, num_verts): 
...
def DFS(graph, v, visited = []):
    "DFS on a graph generated by
    the adjacencyList function
    """
    visited.append(v)
    for w in graph[v]:
        if w not in visited:
            DFS(graph, w, visited)
    return visited # modify to return list of cycles
        

def faceify(out_string, verts, edges): # this one's trickier
    """
    Uses DFS to traverse a graph and construct
    a list of faces in V+E time (woo!).
    """
    DFS(adjacencyList(edges, len(verts), 0)
